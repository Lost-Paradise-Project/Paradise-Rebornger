...
cassette-repair-finish = Вам удается намотать ленту обратно в { THE($item) }.
tape-cassette-position = Кассета прокручена примерно на [color=green]{ $position }%[/color].
tape-cassette-damaged = Лента кассеты размоталась, используйте ручку или отвертку, чтобы починить ее.
...
tape-recorder-locked = Невозможно извлечь кассету, пока диктофон работает.
