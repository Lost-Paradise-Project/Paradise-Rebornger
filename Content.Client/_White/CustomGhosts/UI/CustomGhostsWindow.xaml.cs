...
public sealed partial class CustomGhostsWindow : DefaultWindow
{
    ...
    private Dictionary<string, List<CustomGhostPrototype>> _allGhosts = new(); // won't include ghosts that are uuid-locked
    ...
    //LP edit start
    private void OnCustomGhostAnswer(CustomGhostAnswer ev)
    {
        if (ev.Reasons == null)
            return;

        Dictionary<string, string> results = new();
        foreach ((var proto, var reasons) in ev.Reasons)
        {
            if (reasons == null)
            {
                results.Add(proto, "");
    ...
    private void BuildList()
    {
        ...
        foreach (var category in categories)
        {
            ...
            foreach (var ghostProto in ghosts)
                ...
                {
                    //LP edit end
                    var sponsorrestriction = ghostProto?.Restrictions?.OfType<SponsorRestriction>().FirstOrDefault();
                    button.ActualButton.Disabled = !(sponsorrestriction != null && sponsorrestriction.tier <= SponsorSimpleManager.GetTier() || ghostProto?.ID == "default");

                    if (_reasons != null && ghostProto != null && _reasons.ContainsKey(ghostProto.ID))
                        button.ToolTip = _reasons[ghostProto.ID];
                    //LP edit end
    ...
    private void OnPressed(BaseButton.ButtonEventArgs args)
    {
        ...
        string uuid = _playerManager.LocalUser?.ToString() ?? "";
        _currentGhostProtoId = protoId;
        _currentActive = (Button) args.Button;
        _pref.SetCustomGhost(protoId);
        _netManager.ClientSendMessage(new ChangeCustomGhostMsg { id = protoId, uuid = uuid });
