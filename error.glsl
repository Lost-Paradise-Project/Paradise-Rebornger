...
uniform ARRAY_HIGHP vec3 tint;
uniform ARRAY_HIGHP float luminance_threshold;
uniform ARRAY_HIGHP float noise_amount;
uniform ARRAY_HIGHP float scanline_intensity;
uniform ARRAY_HIGHP float vignette_strength;
uniform ARRAY_HIGHP float flicker_speed;


ARRAY_LOWP float rand( ARRAY_LOWP vec2 co) {
...
void main()
{
 ...
 if ( grey < threshold ) grey += ( threshold - grey ) * 0.5 ;
 ...
 color . rgb += flicker ;
 highp vec2 center = SCREEN_UV - vec2 ( 0.5 ) ;
 highp float radius = length ( center ) ;
 highp float mask = step ( radius , 0.72 ) ;
 highp float feather = smoothstep ( 0.60 , 0.72 , radius ) ;
 color . rgb *= mask ;
 color . rgb *= ( 1.0 - feather * vignette_strength ) ;
 COLOR = color ;
